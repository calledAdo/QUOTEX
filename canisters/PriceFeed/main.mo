...
shared ({ caller }) actor class PriceFeed(xrc : Principal) = {
    ...
    //returns the percentage deviation from the previous price

    private func percentDeviation(prevRate : Nat64, newRate : Nat64) : Nat64 {
        var diff = newRate - prevRate;
        if (prevRate > newRate) {
            diff := prevRate -newRate
        };
        return (diff * 100) / prevRate
    };
    ...
    public shared ({ caller }) func get_exchange_rate(args : XRC.GetExchangeRateRequest) : async XRC.ExchangeRate {
        ...
        if (isElapsed(args.base_asset.symbol) == false) {
            ...
            let previous_rate_data : ExchangeRate = switch (LAST_TRADED_RATE.get(args.base_asset.symbol)) {
                case (? #Ok(res)) { res };
                case (_) {
            ...
            previous_rate := previous_rate_data.rate;
            return previous_rate_data
        };
        ...
        let new_rate_data : ExchangeRate = switch (await getRate(args)) {
            case (#Ok(res)) { res };
            case (#Err(err)) { throw Error.reject("") }
        };
        let percent_deviation : Nat64 = percentDeviation(previous_rate, new_rate_data.rate);
        var heartbeat = last_factor.heartbeat;
        if (percent_deviation > last_factor.deviation) {
            heartbeat := (Nat64.toNat(percent_deviation) * Int.abs(last_factor.heartbeat)) / Nat64.toNat(percent_deviation)
        };
        ASSET_FACTORS.put(args.base_asset.symbol, { deviation = last_factor.deviation; heartbeat = heartbeat; last_updated_time = Time.now() });
        LAST_TRADED_RATE.put(args.base_asset.symbol, #Ok(new_rate_data));
